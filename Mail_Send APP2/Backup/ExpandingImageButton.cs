...
namespace MetaBuilders.WebControls
{
	...
	public class ExpandingImageButton : System.Web.UI.WebControls.WebControl, INamingContainer
	{
		...
		/// </summary>
		[
		System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Design", "CA1056:UriPropertiesShouldNotBeStrings"), Description("Gets or sets the url of the image shown when the target control is in the expanded state."),
		Category( "Appearance" ),
		DefaultValue( "" ),
		Bindable( true ),
		]
		...
		/// </summary>
		[
		System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Design", "CA1056:UriPropertiesShouldNotBeStrings"), Description("Gets or sets the url of the image shown when the target control is in the contracted state."),
		Category( "Appearance" ),
		DefaultValue( "" ),
		Bindable( true ),
		]
		...
		private String ClientExpandedImageUrl
		{
			get
			{
				String imageUrl = this.ExpandedImageUrl;
				if ( String.IsNullOrEmpty( imageUrl ) )
				{
					imageUrl = this.Page.ClientScript.GetWebResourceUrl( typeof( ExpandingImageButton ), "MetaBuilders.WebControls.Embedded.Minus.gif" );
				}
				return imageUrl;
			}
		}

		private String ClientContractedImageUrl
		{
			get
			{
				String imageUrl = this.ContractedImageUrl;
				if ( String.IsNullOrEmpty( imageUrl ) )
				{
					imageUrl = this.Page.ClientScript.GetWebResourceUrl( typeof( ExpandingImageButton ), "MetaBuilders.WebControls.Embedded.Plus.gif" );
				}
				return imageUrl;
			}
		}
		...
		protected override void Render( HtmlTextWriter writer )
		{
			...
			if ( this.Expanded )
			{
				this.button.ImageUrl = this.ClientExpandedImageUrl;
				this.button.AlternateText = this.ExpandedAlternateText;
				this.button.ToolTip = this.ExpandedAlternateText;
			}
			else
			{
				this.button.ImageUrl = this.ClientContractedImageUrl;
